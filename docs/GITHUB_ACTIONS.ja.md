...
交換成功を確認するたびに、**povo2.0 session keeper** は新しい `next_due_at` から次回の正確な cron を自動生成します。主実行は 10 分前に開始し、一時 Runner でセッションを復号・更新します。目標まで 15 分以内の場合だけ Runner 内で待機し、目標分に達してから最大 1 回送信します。目標の 5、15、30 分後にフォールバックがあり、日次の安全確認もあります。

すべての入口が `next_due_at`、一時停止状態、送信状態を確認するため、古い入口やフォールバックで重複交換は発生しません。GitHub cron は待機、遅延、欠落の可能性があり、秒単位の正確さは保証できません。結果を確認できない場合は `unknown` へ移行し、自動再試行を停止します。正確な cron は公開 `.github/workflows/povo.yml` に書かれるため、公開 Fork では次回の目標日と分が表示されます。
