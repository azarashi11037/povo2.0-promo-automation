...
每次确认兑换成功后，**povo2.0 session keeper** 会根据新的 `next_due_at` 自动重写下一轮精确 cron：

1. 主入口在目标分钟之前 10 分钟启动；
2. 在临时 Runner 中解密并刷新会话；
3. 只有距离目标不超过 15 分钟时才在 Runner 内等待，到目标分钟后最多提交一次；
4. 目标后 5、15、30 分钟各有一个补偿入口；
5. 每日另有一次安全检查，用于所有精确入口均被延迟或丢弃的情况；
6. 更新后的会话、状态和下一轮 cron 一起提交，随后销毁明文。

所有入口都会检查加密状态中的 `next_due_at`、暂停状态和提交状态，因此旧入口或补偿入口不会造成重复兑换。GitHub cron 仍可能排队、延迟甚至被丢弃，不能保证秒级准时；若结果无法确认，状态会变为 `unknown` 并阻止自动重试。精确 cron 写在公开的 `.github/workflows/povo.yml` 中，因此公开 Fork 会显示下一次目标日期和分钟。
