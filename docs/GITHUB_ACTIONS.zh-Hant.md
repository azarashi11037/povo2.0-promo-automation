...
每次確認兌換成功後，**povo2.0 session keeper** 會依新的 `next_due_at` 自動重寫下一輪精確 cron。主入口提前 10 分鐘啟動，先在臨時 Runner 解密並更新工作階段；只有距離目標不超過 15 分鐘時才在 Runner 內等待，到目標分鐘後最多提交一次。目標後 5、15、30 分鐘各有補償入口，另有每日安全檢查。

所有入口都會檢查 `next_due_at`、暫停狀態及提交狀態，因此舊入口或補償入口不會重複兌換。GitHub cron 仍可能排隊、延遲或遺失，無法保證秒級準時；結果無法確認時會進入 `unknown` 並停止自動重試。精確 cron 會寫入公開的 `.github/workflows/povo.yml`，所以公開 Fork 會顯示下一次目標日期與分鐘。
